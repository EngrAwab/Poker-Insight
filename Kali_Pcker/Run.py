...
MODEL_PATH = r"D:\Programmes\Freelance\Poker-Insight\Models\kali_pocker_card.pt"
model = YOLO(MODEL_PATH)

# ───────────────────── card inference ─────────────────────
def card_inference(crop_bgr):
    """
    Run the YOLO model on a cropped Player (“P”) rectangle and
    return detections as (x1, y1, x2, y2) in crop coordinates.
    """
    results = model.predict(source=crop_bgr, conf=0.25, verbose=False)
    boxes = []
    if results:
        res = results[0]
        for b in res.boxes:
...
# ╔══════════════════════════ SETTINGS ═════════════════════════╗
VIDEO_PATH   = r"D:\Programmes\Freelance\Poker-Insight\Kali_Pcker\Game\videoplayback.mp4"
OUTPUT_TXT   = r'D:\Programmes\Freelance\Poker-Insight\Kali_Pcker\boxes.txt'

MAX_WIDTH    = 1280
MAX_HEIGHT   = 800
FONT         = cv2.FONT_HERSHEY_SIMPLEX
COORD_SCALE  = 2           # scale factor applied when loading boxes
INFERENCE_SEC = 3          # ← NEW: run card detection every 3 seconds
...
# --- status words ----------------------------------------------------------
KEY_STATUSES = {'fold', 'all in', 'raise', 'bet', 'call', 'check'}
...
def ocr_text(frame, rect, reader):
    """Run EasyOCR on a cropped rectangle; return plain text ('' if nothing)."""
    x1, y1, x2, y2, _ = rect
...
def run_viewer(boxes):
    ...
    # NEW: frames between card-inference runs
    inf_interval_frames = max(1, int(round(fps * INFERENCE_SEC)))
    last_inf_frame = -inf_interval_frames
    card_boxes_cached = []
    ...
    player_status = {}     # {pid: status string}
    round_num = 1
    ...
    while True:
        ...
        # ---------- decide if we will OCR heavy this frame ----------
        ocr_this_frame = (f_idx - last_ocr_frame) >= fps_int
        if ocr_this_frame:
            last_ocr_frame = f_idx

        # ---------- PASS 1: Name / Investment OCR (every frame) ----------
        ...
        # ---------- PASS 2: status / round handling (1×/s) ----------
        ante_detected = False
        ...
        # ---------- NEW: run card_inference every 3 sec ----------
        if (f_idx - last_inf_frame) >= inf_interval_frames:
            card_boxes_cached = []                     # clear & rebuild
            for _, (x1, y1, x2, y2, _) in players_idx:
                crop = frame[y1:y2, x1:x2]
                if crop.size < 10:
                    continue
                for (cx1, cy1, cx2, cy2) in card_inference(crop):
                    # offset from crop → full-frame coords
                    card_boxes_cached.append(
                        (cx1 + x1, cy1 + y1, cx2 + x1, cy2 + y1)
                    )
            last_inf_frame = f_idx
        # else: keep using card_boxes_cached
        ...
        # ---------- NEW: draw cached card detections ----------
        for bx1, by1, bx2, by2 in card_boxes_cached:
            sx1, sy1 = scale_coords(bx1, by1, s)
            sx2, sy2 = scale_coords(bx2, by2, s)
